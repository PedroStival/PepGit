<template>
  ...
  <div class="card-header border-0 py-5 mt-3">
    <h5 class="card-title fw-bolder" v-if="item">Capitulo {{item.capituloNumero}}.{{item.itemNumero}}.{{item.numero}}</h5>
  </div>
  ...
  <div class="row g-6 g-xl-9 mb-6 mb-xl-9 mt-3" v-if="item != null && item.documentos">
      <KTFile
       v-for="doc in item.documentos" :key="doc.id"
       :file-title="doc.nome"
       :file-type="doc.type"
...
export default defineComponent({
  ...
  setup() {
      ...
      ApiService.get("documento/subitem/" + empresaId + "/" + subitemId).then(({ data }) => {
        item.value = data;
      });
